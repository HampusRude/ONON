...
def update_db(rl):
    ...
    sm_res_id = []  # List of ids retrieved from surveymonkey

    # For each response in the list responses, create an object if not already in db, add to db
    for res in rl:
        obj = Responses(**res)
        sm_res_id.append(obj.response_id)
    ...
    for res in db_res_id:
        if res not in sm_res_id:
            obj = Responses.query.filter_by(response_id=res).first()
            print("Removed response %s from database" % (str(obj.response_id)))
            db.session.delete(obj)
    db.session.commit()

### Takes response id, retrieves the response from SM and adds it to database
...
### Retrieves response and answers from json data and returns a dictionaries
